...
describe("Supabase Utils", () => {
  ...
  it("getSweepById fetches a single sweep joined with logs", async () => {
    const mockData = { id: "123", repo_name: "test-repo", shipmate_logs: [] };
    mockSingle.mockResolvedValue({ data: mockData, error: null });

    const { data } = await getSweepById("123");

    expect(mockFrom).toHaveBeenCalledWith("shipmate_sweeps");
    expect(mockSelect).toHaveBeenCalledWith("*, shipmate_logs ( * )");
  ...
  it("insertLog inserts a log entry", async () => {
    ...
    expect(mockFrom).toHaveBeenCalledWith("shipmate_logs");
