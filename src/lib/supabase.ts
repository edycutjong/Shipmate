...
export async function getSweepById(id: string) {
  const { data, error } = await createServerClient()
    .from("shipmate_sweeps")
    .select("*, shipmate_logs ( * )")
    .eq("id", id)
    .single();
  return { data: data as (SweepRow & { shipmate_logs: LogRow[] }) | null, error };
}
...
export async function insertLog(row: Pick<LogInsert, "sweep_id" | "level" | "message">) {
  return createServerClient().from("shipmate_logs").insert(row);
}
